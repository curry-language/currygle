...
parseSignature text = do
  args <- splitFunctionArguments text
 ...
 where
  ...
  splitFunctionArguments (c:cs) = do
    (next, left) <- getNextFunArg (c:cs) [] 0 0
    if length left == 0
      then return [next]
      else do args <- splitFunctionArguments left
              return (next:args)
...
parseList str
  ...
  = Nothing
  | otherwise
  = do sig1 <- parseSignature (removeLastBracket (drop 1 (trimf str)) 1)
       return (Type "[]" [sig1])
 where
...
parseParenth str
  ...
  = let tupleSize = getTupleSize str
    in if tupleSize > 1
         ...
         else do str1 <- removeLastParenth (drop 1 str) [] 1
                 parseSignature str1
...
parseTuple n str = do
  (sigs,_) <- parseTupleElements n (drop 1 str)
  if length sigs == 0
    then Nothing
    else Just (Type ('(' : (take (n-1) (repeat ','))++")") sigs)
 where
  ...
  parseTupleElements x input
    | x == 1 = do (next,left) <- getNextElement input
                  return ([next], left)
    | x >  1 = do (next,left1) <- getNextElement input
                  (sigs,left2) <- parseTupleElements (x-1) left1
                  return (sigs ++ [next], left2)
  ...
  getNextElementRec (x:xs) acc p
    | p == 0 && x `elem` ",)" = do sig <- parseSignature acc
                                   return (sig, xs)
...
parseType str = do
  ...
  if null next
    then Nothing
    else do sigs <- signatruesOfItemForType left
            return $ if Type next sigs == Type "String" []
                       then Type "[]" [Type "Char" []]
                       else Type next sigs
 where
  ...
  signatruesOfItemForType (c:cs) =
    if null (trimf (c:cs))
      then Just []
      else do let (next, left) = getNextElement (c:cs) [] 0 0
              if null (trimf (c:cs))
                then return []
                else do sig  <- parseSignature next
                        sigs <- signatruesOfItemForType left
                        return (sigs ++ [sig])
